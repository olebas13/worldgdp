...
import org.olebas.worldgdp.dao.mapper.CountryRowMapper;
...
public class CountryDAO {
    ...
    private static final String PAGINATION_CLAUSE = " ORDER BY c.code LIMIT :offset , :size ";
    ...
    public List<Country> getCountries(Map<String, Object> params) {
        ...
        System.out.println(SELECT_CLAUSE + " WHERE 1 = 1 "
                ...
                + " ORDER BY c.code "
                + " LIMIT :offset , :size ");
        return namedParameterJdbcTemplate.query(SELECT_CLAUSE + " WHERE 1 = 1 "
                + (!StringUtils.isEmpty((String)params.get("search")) ? SEARCH_WHERE_CLAUSE : "")
                + (!StringUtils.isEmpty((String)params.get("continent")) ? CONTINENT_WHERE_CLAUSE : "")
                + (!StringUtils.isEmpty((String)params.get("region")) ? REGION_WHERE_CLAUSE : "")
    ...
    public int getCountriesCount(Map<String, Object> params) {
        return namedParameterJdbcTemplate.queryForObject("SELECT COUNT(*) FROM country c"
                + " WHERE 1 = 1 "
                + (!StringUtils.isEmpty((String)params.get("search")) ? SEARCH_WHERE_CLAUSE : "")
                + (!StringUtils.isEmpty((String)params.get("continent")) ? CONTINENT_WHERE_CLAUSE : "")
                + (!StringUtils.isEmpty((String)params.get("region")) ? REGION_WHERE_CLAUSE : ""),
    ...
    public Country getCountryDetail(String code) {
        ...
        return namedParameterJdbcTemplate.queryForObject(SELECT_CLAUSE
                + " WHERE c.code = :code", params, new CountryRowMapper());
    }
